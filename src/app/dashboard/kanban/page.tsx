...
function CloseDialog({
    ...
    const canSubmit = challan.trim() || fileData;

    const handleSubmit = () => {
        if (!canSubmit) return;
        onConfirm({
            reason,
            challanNumber: challan.trim() || undefined,
            proofFileData: fileData ?? undefined,
    ...
    return (
        <Dialog open={!!notice} onOpenChange={(open) => { if (!open) onCancel(); }}>
            <DialogContent className="max-w-md">
                <DialogHeader>
                    <DialogTitle>Close Notice — Proof Required</DialogTitle>
                </DialogHeader>
                <div className="space-y-4 py-2">
                    ...
                    <div className="space-y-1.5">
                        <Label>Challan / Reference No. <span className="text-muted-foreground text-xs">(or upload document below)</span></Label>
                        <Input
                            placeholder="e.g., GST/CHALLAN/2024/00123"
                            value={challan}
                    ...
                    <div className="space-y-1.5">
                        <Label>Response Document <span className="text-muted-foreground text-xs">(PDF / Image)</span></Label>
                        <div className="flex items-center gap-3">
                            <label className="flex flex-1 cursor-pointer items-center gap-2 rounded-md border border-dashed border-border bg-muted/30 px-3 py-2.5 text-sm text-muted-foreground hover:bg-muted/50">
                                <input type="file" accept=".pdf,.jpg,.jpeg,.png" className="hidden" onChange={handleFile} />
                                {file ? (
                                    <><X className="h-4 w-4 text-primary" /><span className="text-foreground truncate max-w-[200px]">{file.name}</span></>
                                ) : (
                                    <><Plus className="h-4 w-4" />Upload proof document</>
                                )}
                            </label>
                            {file && (
                                <button className="text-xs text-muted-foreground hover:text-destructive" onClick={() => { setFile(null); setFileData(null); }}>
                                    Remove
                                </button>
                            )}
                        </div>
                    ...
                    {!canSubmit && (
                        <p className="text-xs text-amber-600 dark:text-amber-400 flex items-center gap-1">
                            ...
                            Provide a challan number or upload a proof document to close this notice.
                        </p>
                    )}
                </div>
                <DialogFooter>
    ...
    return (
        <div className="space-y-6">
            ...
            <CloseDialog
                ...
                onConfirm={(payload) => {
                    if (closingNotice) closeMutation.mutate({
                        id: closingNotice.id,
                        closeReason: payload.reason,
                        challanNumber: payload.challanNumber,
                        proofFileData: payload.proofFileData,
